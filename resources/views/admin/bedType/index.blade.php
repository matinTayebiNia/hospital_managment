<div class="">
                ...
                <div class="x_title">
                    @can("create-bed")
                        <div class="container row" style="padding:25px ">
                            <a href="{{route("panel.bedTypes.create")}}" class="btn btn-primary">افزودن نوع تخت </a>
                            <hr class="In_solid">
                        </div>
                    @endcan
                    <div class="row">
                        <div class=" col-sm-12">
                            ...
                            <div class="col-md-3 col-sm-3 col-xs-12 form-group ">
                                ...
                                <input type="text" id="search" wire:model="search"
                                       placeholder="جست و جو بر اساس نام و شماره ..."
                                       class="form-control">
                            </div>
        ...
        <div class="x_panel">
            ...
            <div class="x_content">
                    ...
                    <table class="table table-striped  jambo_table bulk_action">
                            ...
                            <tr class="even pointer">
                                <td colspan="7" class="alert alert-dismissible">
                                    @unless($selectedAll==true)
                                        <span
                                            class="">   شما <strong>{{count($this->selected)}}</strong> تخت از <strong>{{$bedTypes->total()}}</strong>  را انتخاب کرده اید میخواهید همه تخت ها را انتخاب کنید؟</span>
                                        <button class=" btn-link" wire:click="selectAll">انتخاب همه</button>
                                    @else
                                        شما <strong>تمام({{$bedTypes->total()}})</strong> نوع تخت ها را انتخاب کرده اید.
                                    @endif
                                </td>
                            </tr>
                        @endif
                        @forelse($bedTypes as $bed)
                            ...
                            <tr class="even pointer ability_to_select " data-targetId="{{$bed->id}}">
                                ...
                                <td class=" ">{{$bed->code}}</td>
                                <td class=" last">
                                    <div class="btn-group">
                                        <a href="{{route("panel.bedTypes.edit",$bed->id)}}"
                ...
                <div class="">
                    ...
                    @if(!empty($bedTypes))
                        {{ $bedTypes->withQueryString('search')->links()}}
                    @endif
                </div>
            </div>
    ...
    @include("admin.layouts.alerts",["target"=>"نوع تخت"])
</div>
